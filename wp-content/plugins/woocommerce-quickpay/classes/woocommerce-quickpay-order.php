...
class WC_QuickPay_Order extends WC_Order {
	...
	public function get_transaction_basket_params() {
		...
		if ( apply_filters( 'woocommerce_quickpay_transaction_params_basket_apply_fees', true, $this ) ) {
			foreach ( $this->get_items( 'fee' ) as $item_line ) {
				$basket[] = $this->get_transaction_basket_params_fee_helper( $item_line );
			}
		}
	...
	private function get_transaction_basket_params_line_helper( $line_item ) {
		...
		return [
			...
			'item_price' => WC_QuickPay_Helper::price_multiply( $price, $this->get_currency() ),
			'vat_rate'   => $this->get_tax_rate( $line_item ) // Basket item VAT rate (ex. 0.25 for 25%)
		];
	}
	 ...
	 * @return array
	 */
	private function get_transaction_basket_params_fee_helper( $fee_item ) {
		...
		return [
			...
			'item_price' => WC_QuickPay_Helper::price_multiply( $price, $this->get_currency() ),
			'vat_rate'   => $this->get_tax_rate( $fee_item ) // Basket item VAT rate (ex. 0.25 for 25%)
		];
	}
